...
class ReportController extends Controller
{
    public function createAdminReport(Request $request)
    {
        ...
        $request->validate([
            ...
            'end_date' => [
                'nullable',
                'date',
                // Rule::when($request->has('start_date'), function ($attribute, $value, $fail) use ($request) {
                //     if ($request->input('start_date') && $value < $request->input('start_date')) {
                //         $fail($attribute . ' must be after or equal to the start date.');
                //     }
                // }),
            ],
        ]);
    ...
    public function createManagerReport(Request $request)
    {
        ...
        // Save the PDF to the storage disk (e.g., 'public')
        $pdfPath = 'reports/manager_report.pdf';
        Storage::disk('public')->put($pdfPath, $pdf->output());
